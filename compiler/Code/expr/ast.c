...
{
  if (et == EINT)
    return (uint64_t *)"EINT";
  if (et == EADD)
    return (uint64_t *)"EADD";
  if (et == ESUB)
    return (uint64_t *)"ESUB";
  if (et == EMUL)
    return (uint64_t *)"EMUL";
  if (et == EDIV)
    return (uint64_t *)"EDIV";
  if (et == EMOD)
    return (uint64_t *)"EMOD";
  if (et == EVAR)
    return (uint64_t *)"EVAR";
  if (et == ENEG)
    return (uint64_t *)"ENEG";
...
{
  if (is_empty(list_of_tuples))
  {
    return term;
  }
  return triple(fst(get_elt(list_of_tuples)), term, simplify_terms(snd(get_elt(list_of_tuples)), next_elt(list_of_tuples)));
}
